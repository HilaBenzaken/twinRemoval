...
import TwinRemoval.ProteinComparator;
import TwinRemoval.ProteinComperatprByMonomerDirections;
import TwinRemoval.TwinRemover;
...
import TwinRemoval.PopulationSimilarityRateMonitor;
...
public class GAOptimizer extends GAabstruct {
	...
	public void execute() throws IOException {
		...
		//just for testing
		//MONITOR
		//PopulationSimilarityRateMonitor monitor = new PopulationSimilarityRateMonitor(population.size());
		//TWIN REMOVER
		ProteinComparator comperator = new ProteinComperatprByMonomerDirections(0.8);
		TwinRemover twinRemover = new TwinRemover(1 ,population.size(), comperator);

		for (currentGenerarionNum = 0; currentGenerarionNum < config.numberOfGenerations; currentGenerarionNum++) {

			twinRemover.RemoveTwins(population);
